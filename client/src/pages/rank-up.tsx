...
export default function RankUp() {
  ...
  const { data: savedAnalyses = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!selectedAthleteId) return [];
      const response = await fetch(`/api/athletes/${selectedAthleteId}/rank-up-analyses`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Saved Analyses */}
          {selectedAthleteId && savedAnalyses.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Brain className="h-5 w-5" />
                  Saved Analyses for {selectedAthlete?.name}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {savedAnalyses.map((analysis: any, index: number) => (
                    <div
                      ...
                      data-testid={`saved-analysis-${index}`}
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <Badge variant="outline" className="text-xs">
                              {analysis.rankingType === 'world' ? 'World' : 'Olympic'}
                            </Badge>
                            <Badge variant="outline" className="text-xs">
                              {analysis.category}
                            </Badge>
                            <span className="text-sm text-gray-600 dark:text-gray-400">
                              Target: #{analysis.targetRank}
                            </span>
                          </div>
                          <div className="text-xs text-gray-500 dark:text-gray-400">
                            Calculated: {new Date(analysis.createdAt).toLocaleDateString('en-US', {
                              year: 'numeric',
                              month: 'short',
                              day: 'numeric',
                              hour: '2-digit',
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div>
                          <div className="text-gray-600 dark:text-gray-400">Current Rank</div>
                          <div className="font-semibold text-blue-600 dark:text-blue-400">
                            #{analysis.currentRank}
                          </div>
                        </div>
                        <div>
                          <div className="text-gray-600 dark:text-gray-400">Current Points</div>
                          <div className="font-semibold text-green-600 dark:text-green-400">
                            {Number(analysis.currentPoints).toFixed(1)}
                          </div>
                        </div>
                        <div>
                          <div className="text-gray-600 dark:text-gray-400">Target Points</div>
                          <div className="font-semibold text-purple-600 dark:text-purple-400">
                            {Number(analysis.targetPoints).toFixed(1)}
                          </div>
                        </div>
                        <div>
                          <div className="text-gray-600 dark:text-gray-400">Points Needed</div>
                          <div className="font-semibold text-orange-600 dark:text-orange-400">
                            {Number(analysis.pointsNeeded).toFixed(1)}
                          </div>
                        </div>
                      </div>
                      ...
                      {analysis.aiRecommendations && (
                        <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                          <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">AI Strategy:</div>
                          <div className="text-sm text-gray-700 dark:text-gray-300 line-clamp-2">
                            {analysis.aiRecommendations.strategy}
                          </div>
                        </div>
                      )}
                    </div>
