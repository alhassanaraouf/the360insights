...
export default function AthleteHeaderSelector({
  ...
  const filteredAthletes =
    ((athletesData as any)?.athletes || [])?.filter((athlete: any) => {
      // First filter by sport (case-insensitive)
      if (athlete.sport?.toLowerCase() !== selectedSport.toLowerCase())
        return false;
  ...
  const selectedAthlete = filteredAthletes.find(
                ...
                <CommandGroup>
                  {filteredAthletes?.map((athlete: any) => (
                    <CommandItem
                      ...
                      onSelect={() =>
                        handleAthleteSelect(athlete.id.toString())
                      }
